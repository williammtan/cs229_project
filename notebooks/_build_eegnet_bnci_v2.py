"""Build eegnet_kmin_bnci2014001_v2.ipynb: EEGNet (Lawhern 2018) on BNCI2014001
...
V2 difference vs eegnet_kmin_bnci2014001.ipynb: the convex method is no longer the
"traditional" single-admm() solve trained from scratch on calibration features.
Instead it is the **2-stage source-anchored convex** procedure from
reve_kmin_convexnn_v3.ipynb, run on the *pretrained* source EEGNet's penultimate
features:

  Stage 1 (cold): ADMM solve of a Pilanci-Ergen 2-layer ReLU MLP on the full
                  source-group EEGNet features (fit once per LOSO fold). Saves the
                  ADMM primal vars (u, v) for warm-start.
  Stage 2 (warm): per (held-out subject, K>0) re-solve the convex problem on
                  source + weighted calibration features (calibration rows repeated
                  to reach STAGE2_TARGET_MASS of the loss), warm-starting (u, v)
                  from stage 1. Keeps the adaptation convex while anchoring it to
                  the source solution.

fine-tune & LoRA are unchanged (continue from source weights on calib trials). All
four methods now carry source knowledge, so the v1 "convex trains from scratch"
asymmetry is gone. The convex curve's K=0 point is the *stage-1 source convex
model* (a convex 2-layer ReLU MLP head on frozen EEGNet features), a different
classifier than EEGNet's own linear head, so it generally does NOT coincide with
the source/fine-tune/LoRA curves at K=0.

The EEGNet training loop keeps the MPS-collapse guard (NaN / near-chance source
fit -> deterministic CPU retry) from reve_kmin_convexnn_v3.ipynb.
"""
import json
...
C_TITLE = r'''# EEGNet on BNCI2014001 (BCI IV-2a) - source LOSO + calibration-only adaptation (v2: 2-stage convex)
...
**V2 change:** the convex method is the **2-stage source-anchored convex** procedure
from `reve_kmin_convexnn_v3.ipynb`, run on the pretrained source EEGNet's
penultimate features (rather than v1's single-`admm()` solve trained from scratch on
calibration only).

**Per LOSO fold** (held-out subject `ho`, source = the other 8 subjects):
1. Train EEGNet end-to-end on the source group (with its linear head).
2. Evaluate on `ho`'s fixed eval set with **no adaptation** (the `source` curve,
   K-independent - this is the LOSO baseline).
3. Fit the **stage-1** convex model once on the frozen source-EEGNet features.
4. Adapt to `ho` using **only its K calibration trials per class** with three methods:
...
| **convex NN (2-stage)** | stage 2: warm-started convex re-solve on **source + weighted calibration** EEGNet features (calib repeated to `STAGE2_TARGET_MASS`); warm-starts the stage-1 primal vars | stage-1 source convex (warm) |

`K=0` (or a calib set with <2 classes) means *no adaptation*: source / fine-tune /
LoRA all fall back to the source EEGNet prediction (so those three coincide at K=0),
while **convex** falls back to the **stage-1 source convex model** evaluated on the
eval set. Because the stage-1 convex model is a different classifier head (a convex
2-layer ReLU MLP on frozen EEGNet features) than EEGNet's own linear head, the
convex K=0 point generally does **not** coincide with the other three.

**Symmetry vs v1.** In v1 the convex head was trained from scratch on calibration
features only, so it carried no source knowledge and trailed at low K. Here all
four methods are source-anchored: fine-tune and LoRA inherit the source-trained
EEGNet weights, and the 2-stage convex inherits the stage-1 source solution (both
as the ADMM warm start and as source data in the stage-2 objective). The convex
feature standardizer is fit on the source features (`fit_stage1_source`), shared by
stage 2 and eval.
...
# --- adaptation 3: 2-stage source-anchored convex two-layer ReLU MLP, on frozen EEGNet features ---
CVX_N_NEURONS  = 16
CVX_BETA       = 1.0e-3
CVX_RHO        = 0.1
CVX_ADMM_ITERS = 8        # stage-1 (cold, source) ADMM iterations
CVX_PCG_ITERS  = 32
CVX_RANK       = 20
# Stage 2 is a source-anchored adaptation problem, not a target-only refit.
STAGE2_ADMM_ITERS      = 4       # stage-2 (warm-started, source+calib) ADMM iterations
STAGE2_TARGET_MASS     = 0.35    # calibration share of the stage-2 weighted loss
STAGE2_WARM_START_DUAL = False   # stale ADMM duals are diagnostic-only under target shift
...
def admm_warm(model, admm_params, u_init=None, v_init=None, lam_init=None):
    """Inlined copy of cld.optimizers.admm.admm with optional warm-start on
    (u, v, lam). (s, nu) are always re-zeroed because their shape (.., n, ..)
    depends on n_samples, which changes between stage 1 and stage 2. Stores the
    final (u, v, s, lam, nu, theta1, theta2) on the model."""
...
def fit_stage1_source(X: np.ndarray, y: np.ndarray, n_classes: int = N_CLASSES,
                      beta: float = CVX_BETA, seed: int = SEED):
    """Stage 1: cold ADMM on the source-EEGNet penultimate features. Fits a
    StandardScaler on the source features and returns (model, scaler). The model
    keeps (u, v, lam) for warm-starting stage 2."""
    scaler = StandardScaler().fit(X.astype(np.float32))
    Xs = scaler.transform(X.astype(np.float32)).astype(np.float32)
    m = CVX_ReLU_MLP(jnp.asarray(Xs), jnp.asarray(y.astype(np.int32)),
                     n_classes=n_classes, P_S=CVX_N_NEURONS, beta=beta, rho=CVX_RHO,
                     seed=jax.random.PRNGKey(seed))
    m.init_model()
    admm_warm(m, dict(rank=CVX_RANK, beta=beta, gamma_ratio=1.0,
...
def calibration_repeat_count(n_src: int, n_calib: int, target_mass: float = STAGE2_TARGET_MASS) -> int:
    """Integer row-repeat approximating a source/target weighted loss so the
    calibration trials carry ~target_mass of the total mass in stage 2."""
    if n_calib <= 0:
        return 0
...
def fit_stage2_source_anchored(X_src: np.ndarray, y_src: np.ndarray,
                               Xc: np.ndarray, yc: np.ndarray,
                               src_model, src_scaler, n_classes: int = N_CLASSES,
                               beta: float = CVX_BETA, target_mass: float = STAGE2_TARGET_MASS,
                               seed: int = SEED):
    """Stage 2: convex adaptation on source + weighted calibration features,
    warm-starting the primal variables (u, v) from the stage-1 source solve. The
    source solution is part of the objective (not just the ADMM init), so the
    adaptation stays convex while remaining anchored to the source. Repeating
    calibration rows approximates a weighted squared loss without changing CLD.

    `seed` MUST match the stage-1 seed: it sets the random hyperplanes, and the
    warm-started (u, v) are only coherent against the same hyperplane bank."""
    ...
    m = CVX_ReLU_MLP(jnp.asarray(X_aug_s), jnp.asarray(y_aug.astype(np.int32)),
                     n_classes=n_classes, P_S=CVX_N_NEURONS, beta=beta, rho=CVX_RHO,
                     seed=jax.random.PRNGKey(seed))   # same seed -> same hyperplanes as stage 1
    m.init_model()
    admm_warm(m, dict(rank=CVX_RANK, beta=beta, gamma_ratio=1.0,
                      admm_iters=STAGE2_ADMM_ITERS, pcg_iters=CVX_PCG_ITERS, check_opt=False),
              u_init=src_model.u, v_init=src_model.v,
...
def convex_nn_logits(model, scaler: StandardScaler, X: np.ndarray) -> np.ndarray:
    Xs = scaler.transform(X.astype(np.float32)).astype(np.float32)
    return np.asarray(model.stacked_predict(jnp.asarray(Xs), model.theta1, model.theta2))

def convex_nn_predict(model, scaler: StandardScaler, X: np.ndarray) -> np.ndarray:
...
for ho in SUBJECTS:
    ...
    # 2-stage convex: stage 1 (cold) fit once per fold on frozen source-EEGNet features.
    cvx_seed = SEED + ho                                     # shared by stage 1 & stage 2 (same hyperplanes)
    F_src = eegnet_features(src_model, Xs_src)
    cvx_src_model, feat_scaler = fit_stage1_source(F_src, y_src, seed=cvx_seed)
    ...
    cvx_base_acc = float((convex_nn_predict(cvx_src_model, feat_scaler, Fe) == ye).mean())
    print(f"S{ho:02d}: source EEGNet fit {time.time()-t0:.1f}s  LOSO(K=0)={base_acc:.3f}  "
          f"convex-stage1(K=0)={cvx_base_acc:.3f}")
    ...
    for k in K_BUDGETS:
        ...
        # 3) 2-stage source-anchored convex NN on frozen source-EEGNet features.
        #    Non-adaptable (K=0) -> stage-1 source convex model (its own baseline,
        #    a convex head != EEGNet's linear head, so it need not match base_acc).
        if not adaptable:
            cvx_acc = cvx_base_acc
        else:
            try:
                Fc = eegnet_features(src_model, Xc)
                cvx_model = fit_stage2_source_anchored(F_src, y_src, Fc, yc,
                                                       cvx_src_model, feat_scaler, seed=cvx_seed)
                cvx_acc = float((convex_nn_predict(cvx_model, feat_scaler, Fe) == ye).mean())
            except Exception as e:
                warnings.warn(f"convex stage-2 failed S{ho:02d} K={k}: {type(e).__name__}: {e}")
                cvx_acc = cvx_base_acc
...
STYLES = [("source",   "source EEGNet (no adapt)",              "x", "--"),
          ...
          ("convex",   "ConvexNN 2-stage (source-anchored)",    "s", "-")]
...
ax.set_title(f"BNCI2014001 (BCI IV-2a) - EEGNet source LOSO + calibration-only adaptation (v2)\n"
             f"({len(SUBJECTS)}-fold LOSO; convex = 2-stage source-anchored)")
...
C_SAVE = r'''out_dir = REPO_ROOT / "results" / "eegnet_bnci2014001_kmin_v2_nb"
...
cells = [
    ...
    code(C_EEGNET),
    md("## 3. 2-stage source-anchored convex two-layer ReLU MLP (warm-startable ADMM)"),
    code(C_CONVEX),
    ...
    code(C_SPLIT),
    md("## 7. Full LOSO + calibration-only adaptation sweep (source / fine-tune / LoRA / 2-stage convex)"),
    code(C_SWEEP),
...
out = NB / "eegnet_kmin_bnci2014001_v2.ipynb"
